...
def remove_spikes(sequence: NDArray[np.float64], iterations: int = 1) -> NDArray[np.float64]:
...
def correlation(model_y: NDArray[np.float64],
                another_x: NDArray[np.float64], another_y: NDArray[np.float64]) -> NDArray[np.float64]:
    from scipy.signal import butter, lfilter  # type: ignore

    def butter_bandpass_filter(data: NDArray[np.float64],
                               low_cut: float, high_cut: float, order: int = 5) -> NDArray[np.float64]:
        def butter_bandpass() -> tuple[NDArray[np.float64], NDArray[np.float64]]:
            ...
            if low > 0. and high >= fs:
                return cast(tuple[NDArray[np.float64], NDArray[np.float64]], butter(order, low, btype='highpass'))
            if low <= 0. and high < fs:
                return cast(tuple[NDArray[np.float64], NDArray[np.float64]], butter(order, high, btype='lowpass'))
            raise ValueError
    ...
    if another_y.size:
        ...
        another_y_filtered: NDArray[np.float64] = butter_bandpass_filter(another_y,
...
def peaks_positions(data_x: NDArray[np.float64], data_y: NDArray[np.float64],
                    threshold: float = 0.0046228) -> NDArray[np.float64]:
    import pandas as pd  # type: ignore
    if data_x.size < 2 or data_y.size < 2:
        # nothing to do
        return np.empty(0)

    std: NDArray[np.float64] = pd.Series(data_y).rolling(int(round(LINE_WIDTH / (data_x[1] - data_x[0]))),
    ...
    peaks: NDArray[np.float64] = np.array([i[0] + np.argmax(data_y[i[0]:i[1]])
                                           for i in islands
