...
def from_iso_format(s: str) -> datetime:
    if sys.version_info < (3, 11, 0):
        ...
        if s.isdigit() and len(s) == 8:  # '20111104'
            ...
            s = (
                "-".join((s[:4], s[4:6], s[6:8]))
                + s[8]
                + ":".join((s[9:11], s[11:13], s[13:]))
            )
    return datetime.fromisoformat(s)


def get_github_date(
    user: str, repo_name: str, branch: str = "master"
) -> datetime | None:
    import json
...
def upgrade_files(
    code_directory: Path, user: str, repo_name: str, branch: str = "master"
) -> bool:
    ...
    with zipfile.ZipFile(io.BytesIO(r.read())) as inner_zip:
        ...
        for member in inner_zip.infolist():
            ...
            if member.is_dir():
                ...
                continue
            (code_directory / Path(member.filename).relative_to(root)).parent.mkdir(
                parents=True, exist_ok=True
            )
            (code_directory / Path(member.filename).relative_to(root)).write_bytes(
                inner_zip.read(member)
            )
            logger.info(
                f"{(code_directory / Path(member.filename).relative_to(root))} written"
            )
...
def update_from_github(user: str, repo_name: str, branch: str = "master") -> bool:
    with suppress(Exception):
        ...
        if (
            version_path.exists()
            and datetime.fromtimestamp(version_path.stat().st_mtime) >= github_date
        ):
            logger.info("Current files are up to date")
            return False

        if upgrade_files(
            code_directory=code_directory, user=user, repo_name=repo_name, branch=branch
        ):
            # if everything went fine...
            version_path.parent.mkdir(exist_ok=True, parents=True)
            version_path.write_text(
                f'__version__ = version = "{github_date.isoformat()}"\n'
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "branch", type=str, help="the GitHub repository branch", default="master"
    )
